...
** Tested on
   ...
   my MacBook Pros with Big Sur installed.

   There are a few things that are currently hard-coded to Big Sur. For example
   the expected system Python version is 3.8 (which is what comes with Big Sur),
   so things are mostly expected to work best with Big Sur. I should come around
   and fix this expectation whenever I get some time. Feel free to open an issue
   if you're seeing issues with earlier macOS versions.
...
*** Clone repository
    Now your machine should have =git= installed.
...
*** Provision machine
    ...
    If a task fails you can either disable the role via =--skip-tags= or use
    =--start-at-task 'Some task name'= and replace =Some task name= with the =name=
    value of some task to cause Ansible to start the playbook exactly there.
    Check the [[https://docs.ansible.com/ansible/latest/user_guide/playbooks_startnstep.html][official documentation]] for more details.
